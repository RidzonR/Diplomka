...
namespace UnitySharpNEAT
{
    ...
    /// </summary>
    [Serializable]
    public class CoroutinedListEvaluatorCustom<TGenome, TPhenome> : IGenomeListEvaluator<TGenome>
        ...
        where TPhenome : class
    {
        ...
        /// </summary>
        public CoroutinedListEvaluatorCustom(IGenomeDecoder<TGenome, TPhenome> genomeDecoder,
                                         IPhenomeEvaluator<TPhenome> phenomeEvaluator,
                                          NeatSupervisorCustom neatSupervisor)
        {
        ...
        private IEnumerator EvaluateList(IList<TGenome> genomeList)
        {
            ...
            for (int i = 0; i < _neatSupervisor.Trials; i++)
            {
                ...
                // add here check for all cars dead to start earlier
                for (int j = 0; j < _neatSupervisor.TrialDuration; j++)
                {
                    yield return new WaitForSeconds(1f);
                    if (!_neatSupervisor.CheckActiveUnits())
                        break;
                }
            ...
            foreach (TGenome genome in dict.Keys)
            {
                ...
                if (phenome != null)
                {
                    ...
                    if (fitness > _neatSupervisor.StoppingFitness)
                    {
                        ...
                        _phenomeEvaluator.StopConditionSatisfied = true;    // reason stoping fittnes work no more
                    }
